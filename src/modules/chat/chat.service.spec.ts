...
describe('ChatService', () => {
  ...
  const testUser = { id: 'user-1', email: 'user1@test.com' };
  ...
  it('should call free LLM and return response', async () => {
    ...
    const result = await service.sendMessage('Explain quantum computing', [], testUser);
  ...
  it('should accept conversation history', async () => {
    ...
    const result = await service.sendMessage('Tell me more', history, testUser);
  ...
  it('should fall back to keyword responses when LLM fails', async () => {
    ...
    const result = await service.sendMessage('What are the top news today?', [], testUser);
  ...
  it('should fall back for finance queries when LLM fails', async () => {
    ...
    const result = await service.sendMessage('How is the market doing?', [], testUser);
  ...
  it('should fall back for weather queries when LLM fails', async () => {
    ...
    const result = await service.sendMessage('What is the weather like?', [], testUser);
  ...
  it('should fall back for schedule queries when LLM fails', async () => {
    ...
    const result = await service.sendMessage('What meetings do I have today?', [], testUser);
  ...
  it('should fall back for email queries when LLM fails', async () => {
    ...
    const result = await service.sendMessage('Check my inbox', [], testUser);

    expect(result.message).toMatch(/email|Email Summaries/i);
  });

  it('should handle empty message', async () => {
    const result = await service.sendMessage('', [], testUser);
  ...
  it('should clean response prefixes', async () => {
    ...
    const result = await service.sendMessage('Test', [], testUser);
  ...
  it('should handle null LLM response and fall back', async () => {
    ...
    const result = await service.sendMessage('Hello there', [], testUser);
